...
use pgrust::{
    connection::{
        params::{Host, HostType},
        parse_postgres_dsn_env,
        tokio::TokioSocketAddress,
        Client, Credentials,
    },
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut socket_address: Option<TokioSocketAddress> = None;
    if let Some(dsn) = args.dsn {
        ...
        let host = conn.hosts.first().unwrap();
        eprintln!("Connecting to {host:?}");
        match host {
            Host(HostType::Path(path), port) => {
                socket_address = Some(TokioSocketAddress::new_unix(format!(
                    "{path}/.s.PGSQL.{port}"
                )));
            }
            Host(HostType::Hostname(hostname), port) => {
                let addr = format!("{hostname}:{port}")
                    .to_socket_addrs()?
                    .next()
                    .unwrap();
                socket_address = Some(TokioSocketAddress::new_tcp(addr));
            }
            Host(HostType::IP(ip, interface), port) => {
                let addr = if let Some(interface) = interface {
                    format!("{ip}%{interface}")
                } else {
                    ip.to_string()
                };
                socket_address = Some(TokioSocketAddress::new_tcp(
                    format!("{addr}:{port}").parse()?,
                ));
            }
            Host(HostType::Abstract(..), _) => {
                unimplemented!("Abstract socket connection not yet supported")
            }
        }
    }

    let socket_address = socket_address.unwrap_or_else(|| match (args.tcp, args.unix) {
        (Some(addr), None) => TokioSocketAddress::new_tcp(addr),
        (None, Some(path)) => TokioSocketAddress::new_unix(path),
...
async fn run_queries(
    socket_address: TokioSocketAddress,
