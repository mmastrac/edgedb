...
pub trait Struct {
    type Struct<'a>;
    fn new<'a>(buf: &'a [u8]) -> Self::Struct<'a>;
}

/// Implemented for all generated structs that have a [`Length`] field at a fixed offset.
pub trait StructLength: Struct {
    fn length_field_of<'a>(of: &Self::Struct<'a>) -> usize;
