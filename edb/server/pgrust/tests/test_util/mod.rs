...
use pgrust::connection::{dsn::parse_postgres_dsn, parse_postgres_dsn_env};
...
pub(crate) fn test(
    ...
    if no_env {
        ...
        let mut ours: HashMap<String, String> = ours.into();
    ...
    if expect_mismatch {
        ...
        if no_env {
            ...
            // from the test
            if ours_no_env.ports == Some(vec![Some(5432)]) {
                ours_no_env.ports = None
            }
            assert_eq_map!(
                libpq,
                ours_no_env,
