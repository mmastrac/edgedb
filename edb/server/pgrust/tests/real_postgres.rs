...
use pgrust::connection::tokio::TokioSocketAddress;
use pgrust::connection::{connect_raw_ssl, Authentication, ConnectionSslRequirement, Credentials};
use rstest::rstest;
...
fn postgres_bin_dir() -> std::path::PathBuf {
    Path::new("../../../build/postgres/install/bin")
        .canonicalize()
        .expect("Failed to canonicalize postgres bin directory path")
}
...
async fn test_auth_real(
    ...
    let initdb = postgres_bin_dir().join("initdb");
    let postgres = postgres_bin_dir().join("postgres");
    ...
    let socket_address = match mode {
        Mode::Unix => {
            let path = get_unix_socket_path(&data_dir, port);
            TokioSocketAddress::new_unix(path)
        }
        Mode::Tcp | Mode::TcpSsl => TokioSocketAddress::new_tcp((Ipv4Addr::LOCALHOST, port).into()),
    };
